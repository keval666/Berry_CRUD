...
const SamplePage = () => {
    ...
    const columns = [
        ...
        { title: "User Photo", field: "image", render: item => <Link to="/pages/product"><img src={item.image} alt="" style={{ height: '50px', borderRadius: '50%' }} /></Link> },
        { title: "User Name", field: "username" },
        { title: "Quantity", field: "roll", render: item => <span style={{ backgroundImage: "linear-gradient(195deg, #66BB6A 0%, #43A047 100%)", color: 'white', padding: "3px 8px", borderRadius: '5px' }}>{item.roll}</span> }
    ]

    return (
        <MainCard title={<div className="User_header">Product List Page
            <div>
            ...
            <NavLink to='/pages/userlist/editUser'><button type="button" className="btn add_btn"><AddBox /> Add</button></NavLink></div>}>
            <MaterialTable
                title={<>Product List</>}
                icons={tableIcons}
                data={tableData}
