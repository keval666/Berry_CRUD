// import Product from "context/CRUDcontext"
...
const SamplePage = () => {
    ...
    const deleteRow = async (e) => {
        const data = await CRUD.deleteData(e.id, "Productlist");
        getApiData();
        console.log(e);
        toast.success(`${e.name}'s Data was Deleted `);
    }

    //==================

    const handleEditUser = (data) => {
        navigate(`/firebase/productlist/EditProduct`,
            {
                state: {
                    data: data,
                }
    ...
    const columns = [
        ...
        { title: "Type", field: "type", render: item => <span style={{ backgroundImage: "linear-gradient(195deg, #66BB6A 0%, #43A047 100%)", color: 'white', padding: "3px 8px", borderRadius: '5px' }}>{item.type}</span> },
        { title: "Price", field: "price", render: item => <span>{item.price} ₹</span> },
        { title: "Gst", field: "gst", render: item => <span>{item.gst} %</span> }
    ];
    ...
    return (
        <>
            ...
            <MainCard
            // title={<div className="User_header">Product List
            //     <div>
            ...
            //     </div>
            //     <NavLink to='/firebase/userlist/editUser'><button type="button" className="btn add_btn"><AddBox /> Add</button></NavLink></div>}
            >
                <MaterialTable
                    title={<NavLink to='/firebase/productlist/AddProduct'><button type="button" className="btn btn-primary"><AddBox /> Add New Product</button></NavLink>}
                    icons={tableIcons}
                    data={tableData}
                    columns={columns}
                    isLoading={loading}
